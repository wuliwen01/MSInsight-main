                                    ...
                                    float *result)
{
    ...
    if (n_samples <= 0)
    {
        std::cerr << "[Error] n_samples must be > 0, got " << n_samples << std::endl;
        return -1;
    }
    if (n_samples > 1024)
    {
        std::cerr << "[Error] n_samples exceeds max block size (1024), got " << n_samples << std::endl;
        return -1;
    }
    if (n_sta <= 0 || n_grid <= 0 || n_fm <= 0)
    {
        std::cerr << "[Error] Invalid parameters: n_sta=" << n_sta << ", n_grid=" << n_grid << ", n_fm=" << n_fm << std::endl;
        return -1;
    }
    ...
    std::cout << "[SSA.cu] allocate GPU memory: " << n_sta * n_samples * sizeof(float) + n_sta * n_grid * sizeof(int) + n_fm * n_grid * n_sta * sizeof(float) + n_grid * n_fm * n_samples * sizeof(float) << std::endl;
